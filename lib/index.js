...
exports.parseXML = function(xml) {
  ...
  if(xml instanceof libxml.Document) {

    /** We are already dealing with a libxmljs Document, so just return the root. **/
    return xml.root();

  } else if(xml instanceof libxml.Element) {

    /** We are dealing with a libxmljs Element, so just return it. */
    return xml;
